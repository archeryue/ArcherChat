...
| 11b | DDP path (Stage 3 prerequisite) | ✅ **PASS** | `DistMuonAdamW` vs verified `MuonAdamW` **max\|Δ\| 2.98e-08** at ws=4 across ZERO-RANK/remainder/even shard regimes (`oracle_ddp_check.py`); d8 full run at ws=4 → val_bpb **0.9307** vs oracle **0.9376**; per-rank optimizer shards + `evaluate_bpb` all-reduce green. Found and fixed: `prune_checkpoints` DDP race (leaked shards) and MFU missing the `world_size` factor. Runbook: [STAGE3.md](STAGE3.md) |
